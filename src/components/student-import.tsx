...
export function StudentImport() {
  ...
  if (result) return <section className="card import-complete">
    ...
    <div className="heading-actions"><Link href="/students" className="button button-primary">Ver alumnos</Link><button className="button button-secondary" onClick={() => { setResult(null); setFile(null); }}>Importar otro archivo</button></div>
  </section>;

  return <form className="card import-card" onSubmit={submit}>
    <div className="form-section-head"><span>CSV</span><div><h2>Selecciona el archivo</h2><p>Los alumnos existentes se actualizarán usando su matrícula. Los nuevos quedarán listos para generar su credencial.</p></div></div>
    ...
    <div className="form-actions"><Link href="/students" className="button button-secondary">Cancelar</Link><button className="button button-primary" disabled={!file || loading}>{loading ? "Importando…" : "Importar alumnos"}</button></div>
  </form>;
}
